...
struct SM120ArchSpec {
    ...
    static int get_smem_d_size_for_swizzle(const SM120GemmDesc& desc, const Layout& layout, int swizzle_cd, int store_m) {
    ...
    static int get_smem_per_stage(const SM120GemmDesc& desc, const Layout& layout) {
    ...
    static StorageConfig get_storage_config(const SM120GemmDesc& desc, const Layout& layout) {
    ...
    static int get_smem_d_size(const SM120GemmDesc& desc, const Layout& layout) {
    ...
    static PipelineConfig get_pipeline_config(const SM120GemmDesc& desc, const Layout& layout, const StorageConfig& storage_config) {
        constexpr int kNumMaxStages = 16;
        ...
        const int smem_tensormap =
            is_k_grouped_contiguous(desc.gemm_type) ? 2 * static_cast<int>(sizeof(CUtensorMap)) : 0;
    ...
    static LaunchConfig get_launch_config(const SM120GemmDesc& desc, const Layout& layout) {
    ...
    static LayoutInfo get_layout_info(const SM120GemmDesc& desc, const Layout& layout) {
    ...
    static int get_split_k_factor(const SM120GemmDesc& desc, const Layout& layout) {

...
void sm120_fp4_mqa_logits(const uint32_t seq_len, const uint32_t seq_len_kv,
    ...
    // Block-scaled MMA folds SF into computation — no post-MMA scale multiply.
    ...
    auto smem_sf_kv = utils::PatternVisitor([&](const uint32_t& i) {
        return reinterpret_cast<uint32_t*>(smem_buffer +
            SMEM_Q_SIZE_PER_STAGE * kNumQStages + SMEM_SF_Q_SIZE_PER_STAGE * kNumQStages +
            SMEM_WEIGHT_SIZE_PER_STAGE * kNumQStages +
            SMEM_KV_SIZE_PER_STAGE * kNumKVStages + SMEM_SF_KV_SIZE_PER_STAGE * i);
    });
    ...
    auto barrier_ptr = reinterpret_cast<Barrier*>(
        ...
        SMEM_Q_SIZE_PER_STAGE * kNumQStages + SMEM_SF_Q_SIZE_PER_STAGE * kNumQStages +
        SMEM_WEIGHT_SIZE_PER_STAGE * kNumQStages +
        SMEM_KV_SIZE_PER_STAGE * kNumKVStages + SMEM_SF_KV_SIZE_PER_STAGE * kNumKVStages);
    ...
    if (threadIdx.x >= kNumMathThreads) {
            ...
            while (block_q_idx < num_q_blocks) {
                ...
                for (uint32_t kv_block_idx = 0; kv_block_idx < num_kv_blocks; ++ kv_block_idx) {
                    ...
                    tma::copy<BLOCK_KV, 1, 0>(
                        ...
                        smem_sf_kv[kv_stage_idx], kv_start + kv_block_idx * BLOCK_KV, 0);
        ...
        while (block_q_idx < num_q_blocks) {
            ...
            for (uint32_t kv_block_idx = 0; kv_block_idx < num_kv_blocks; ++ kv_block_idx) {
                ...
                // Load KV SF (packed int32, 4 UE8M0 bytes per row)
                // SFA thread mapping: group_id + (thread_id & 1) * 8 covers 16 M-rows
                const uint32_t sf_kv_packed = sm120::load_sf(
                    reinterpret_cast<const char*>(smem_sf_kv[kv_stage_idx]),
                    warp_idx * MMA_M + g + (t & 1) * 8);
                ...
                for (uint32_t qi = 0; qi < BLOCK_Q; ++ qi) {
                    ...
                    for (uint32_t nt = 0; nt < kNTilesPerQ; ++ nt) {
                        ...
                        // Load Q SF: SFB thread mapping uses group_id for N-tile
                    ...
                    // No post-MMA KV scale — block_scale handles it
                    float v_0 = partial_0;
                    float v_1 = partial_1;

...
namespace deep_gemm::sched_sm120 {
    ...
    CUTLASS_DEVICE bool get_next_block(uint32_t& m_block_idx, uint32_t& n_block_idx) {
        ...
        if constexpr (kGemmType == GemmType::MGroupedMasked) {
            ...
            while (true) {
                ...
                // Move to check the next group
                current_sf_k_cumsum += math::ceil_div(
                    kGemmType == GemmType::KGroupedContiguousWithPsumLayout ? math::align(current_shape_k, kKAlignment) : current_shape_k,
                    kSFKSpan);
